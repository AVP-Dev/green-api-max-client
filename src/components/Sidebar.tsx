...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-full h-full flex flex-col bg-white dark:bg-slate-900 shrink-0 select-none">
      ...
      {pollingStatus === 'error' && (
        <div className="mx-3 my-2 p-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200/80 dark:border-rose-800 flex flex-col gap-1.5 text-[11px] text-rose-900 dark:text-rose-200 animate-in fade-in duration-200">
          <div className="flex items-start gap-1.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <span className="text-rose-700 dark:text-rose-300 break-words [overflow-wrap:anywhere]">
          ...
          <div className="flex items-center justify-end gap-1.5 pt-0.5">
            <button
              ...
              onClick={handleRetry}
              disabled={isRetrying}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 font-medium hover:bg-rose-100/70 dark:hover:bg-rose-950/50 transition-colors cursor-pointer shadow-2xs text-[11px]"
            >
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-slate-100/80 dark:divide-slate-800">
        {isSyncingMessages && dialogs.length === 0 ? (
          ...
          <div className="h-full flex flex-col items-center justify-center p-6 text-center text-slate-400 dark:text-slate-500">
            <div className="w-12 h-12 rounded-2xl bg-indigo-50/70 dark:bg-indigo-950/50 flex items-center justify-center text-[#471AFF] dark:text-indigo-300 mb-3">
              <Sparkles className="w-5 h-5" />
            </div>
            <h4 className="font-semibold text-xs text-slate-700 dark:text-slate-200 mb-1">
              {searchQuery ? (lang === 'ru' ? 'Ничего не найдено' : 'No chats found') : t.noChatsTitle}
            </h4>
            <p className="text-[11px] text-slate-400 dark:text-slate-500 leading-relaxed max-w-[220px] mb-4">
              {searchQuery 
            ...
            {searchQuery ? (
              <button
                ...
                className="px-3.5 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
              >
                <X className="w-3.5 h-3.5" />
              ...
              <button
                ...
                type="button"
                onClick={onOpenNewChat}
                className="px-3.5 py-1.5 bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 dark:hover:bg-indigo-900 text-[#471AFF] dark:text-indigo-300 text-xs font-semibold rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
              >
          ...
          sortedAndFilteredDialogs.map((dialog) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    {typingChats[dialog.chatId] ? (
                      <p className="text-[11px] font-semibold text-[#471AFF] dark:text-indigo-300 flex items-center gap-1 leading-relaxed">
                        <span>{t.typing}</span>
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate leading-relaxed">
                        {dialog.lastMessageDirection === 'outgoing' && (
                          <span className="text-slate-400 dark:text-slate-500 font-medium mr-0.5">{t.youPrefix}</span>
                        )}
                ...
                <div className="hidden md:group-hover:flex transition-opacity absolute right-2 top-2 items-center gap-0.5 bg-white/95 dark:bg-slate-800/95 backdrop-blur-xs rounded-lg shadow-xs border border-slate-200/80 dark:border-slate-700 p-0.5 z-10">
                  {onTogglePinChat && (
                    <button
                      ...
                      className={`p-1.5 sm:p-1 rounded-md transition-colors cursor-pointer ${
                        dialog.isPinned
                          ? 'text-[#471AFF] dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/60'
                          : 'text-slate-400 hover:text-[#471AFF] dark:hover:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/60'
                      }`}
