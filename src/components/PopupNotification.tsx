...
export const PopupNotification: React.FC<PopupNotificationProps> = ({
  ...
  const displayPhone = formatDisplayPhone(notification.chatId);
  const displayName = notification.senderName || displayPhone;

  return (
    <div className="fixed top-4 right-4 z-50 max-w-sm w-full pointer-events-none px-3 sm:px-0">
      <AnimatePresence>
        <motion.div
          key={notification.id}
          ...
          className="pointer-events-auto bg-white/95 backdrop-blur-md rounded-2xl shadow-xl border border-indigo-100 p-3.5 flex flex-col gap-2.5 cursor-pointer hover:shadow-2xl transition-shadow group"
          onClick={() => onOpenChat(notification.chatId)}
        >
          {/* Top Row: App info, badge & close button */}
          <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2">
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[#471AFF] animate-pulse" />
              <span className="text-[11px] font-bold tracking-wide uppercase text-[#471AFF] flex items-center gap-1">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[12px] text-slate-700 mt-1 line-clamp-2 leading-snug font-normal break-words [overflow-wrap:anywhere]">
                {notification.text}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-1">
            {canRequestPermission ? (
              <button
                ...
                className="text-[10px] text-indigo-600 hover:text-indigo-800 flex items-center gap-1 font-medium hover:underline cursor-pointer"
              ...
              <span className="text-[10px] text-slate-400">
                {isRu ? 'Нажмите, чтобы ответить' : 'Click to reply'}
              </span>
            )}
            ...
            <button
              ...
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-gradient-to-r from-[#471AFF] to-indigo-600 text-white text-[11px] font-semibold hover:opacity-95 active:scale-95 transition-all shadow-xs cursor-pointer"
            >
